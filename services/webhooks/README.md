...
| `WEBHOOK_SECRET` | `Attach8-Catfight-...` | HMAC partage avec GitHub (`openssl rand -hex 32`), doit etre identique a ce que tu poses dans Settings > Webhooks > Secret cote repo. Alias accepte : `SECRET` (legacy) |
...
1. Dans Infisical, cree le sous-dossier `/services/webhooks/<nom>/` avec `REPO`, `WEBHOOK_SECRET`, `SCRIPT`.
...
- `/etc/secrets/webhooks/<repo>.env` : REPO, WEBHOOK_SECRET, SCRIPT par repo
